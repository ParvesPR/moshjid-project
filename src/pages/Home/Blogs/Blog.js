...
const Blog = ({ blogs, refetch }) => {
    ...
    const deleteConfirm = (id) => {
        Swal.fire({
            ...
            if (result.value) {
                const url = ` https://obscure-sierra-27359.herokuapp.com/blogs/${id}`;
                console.log(url);
                fetch(url, {
                    method: 'DELETE',
                    headers: {
    ...
    return (
        <div className="card bg-base-100 shadow-xl col-start-3 col-span-4 w-full lg:w-3/4 mx-auto">
            <figure>
                <img className='w-3/4 lg:w-2/4 mx-auto' src={image} alt="card-img" />
            </figure>
